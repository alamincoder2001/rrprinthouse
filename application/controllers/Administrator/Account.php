        ...
        $transactions = $this->db->query("
            select * from(
                ...
                select 
                    'f' as sequence,
                    sa.id,
                    concat('Salary Advance- ',e.Employee_Name, ' - ' ,m.month_name) as description,
                    sa.bank_id as account_id,
                    sa.date as transaction_date,
                    'withdraw' as transaction_type,
                    0.00 as deposit,
                    sa.amount as withdraw,
                    sa.note,
                    ac.account_name,
                    ac.account_number,
                    ac.bank_name,
    ...
    public function getCashLedger() {
        ...
        $ledger = $this->db->query("
            ...
            select 
                ...
                concat('Salary Advance- ',e.Employee_Name, ' - ' ,m.month_name) as description,
                0.00 as in_amount,
                sa.amount as out_amount
            from tbl_salary_advance sa
            join tbl_employee e on e.Employee_SlNo = sa.employee_id
            join tbl_month m on m.month_id = sa.month_id
            where sa.branch_id = '$this->brunch'
            and sa.status = 'a'
            and sa.type = 'cash'
            and sa.date between '$data->fromDate' and '$data->toDate'
