...
class Employee extends CI_Controller
{
    ...
    public function getEmployeesForSalary(){
        ...
        $employees = $this->db->query("
            SELECT 
                ...
                ds.Designation_Name,
                (
                    select ifnull(sum(sa.amount), 0) from tbl_salary_advance sa
                    where sa.employee_id = e.Employee_SlNo
                    and sa.status = 'a'
                    and sa.month_id = " . $data->monthId . "
                    and sa.branch_id = " . $this->session->userdata('BRANCHid') . "
                ) as advance_salary,
                (select e.salary_range - advance_salary) as net_payable,
                concat(e.Employee_Name, ' - ', e.Employee_ID) as display_name
            from tbl_employee e 
            join tbl_department dp on dp.Department_SlNo = e.Department_ID
            join tbl_designation ds on ds.Designation_SlNo = e.Designation_ID
    ...
    public function salaryAdvance()
    {
        ...
        $data['content'] = $this->load->view('Administrator/employee/salary/advance', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addSalaryAdvance()
    {
        $res = ['success'=>false, 'message'=>''];
        try{
            $advanceObj = json_decode($this->input->raw_input_stream);

            $advance = (array)$advanceObj;
            $advance['code'] = $this->mt->generateSalaryAdvanceCode();
            $advance['status'] = 'a';
            $advance['add_by'] = $this->session->userdata("FullName");
            $advance['add_time'] = date('Y-m-d H:i:s');
            $advance['branch_id'] = $this->session->userdata('BRANCHid');

            $this->db->insert('tbl_salary_advance', $advance);
            $paymentId = $this->db->insert_id();

            $res = ['success'=>true, 'message'=>'Salary Advance Added', 'paymentId' => $paymentId];
        } catch (Exception $ex){
    ...
    public function updateSalaryAdvance()
    {
        $res = ['success'=>false, 'message'=>''];
        try{
            $advanceObj = json_decode($this->input->raw_input_stream);
            $paymentId = $advanceObj->id;

            $advance = (array)$advanceObj;
            unset($advance['id']);
            $advance['update_by'] = $this->session->userdata("FullName");
            $advance['update_time'] = date('Y-m-d H:i:s');

            $this->db->where('id', $paymentId)->update('tbl_salary_advance', $advance);

            $res = ['success' => true, 'message' => 'Salary Advance Updated', 'paymentId' => $paymentId];
    ...
    public function getSalaryAdvance()
    {
        ...
        if(isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != ''){
            $clauses .= " and sa.date between '$data->dateFrom' and '$data->dateTo'";
        }

        if(isset($data->type) && $data->type != ''){
            $clauses .= " and sa.type = '$data->type'";
        }
        ...
        $payments = $this->db->query("
            ...
            join tbl_employee e on e.Employee_SlNo = sa.employee_id
            join tbl_month m on m.month_id = sa.month_id
            left join tbl_bank_accounts ba on ba.account_id = sa.bank_id
            where sa.status = 'a'
            and sa.branch_id = ? 
            $clauses
            order by sa.id desc
    ...
    public function deleteSalaryAdvance()
    {
        ...
        try{
            ...
            $this->db->set(['status'=>'d'])->where('id', $data->paymentId)->update('tbl_salary_advance');
            
            $res = ['success' => true, 'message' => 'Salary Advance Deleted successfully'];
    ...
    function advanceSalaryReport($id = Null)
    {
        $data['title'] = "Salary Advance Payment Invoice";
        if ($id != 'pr') {
            $pid["PamentID"] = $id;
            $this->session->set_userdata($pid);
        }
        $data['content'] = $this->load->view('Administrator/employee/salary/advanceInvoice', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
}
