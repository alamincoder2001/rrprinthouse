...
class Order extends CI_Controller {
    ...
    public function updateOrder(){
        ...
        try{
            ...
            if($data->sales->payment_type == 'Bank') {
                if(isset($data->sales->account_id) && $data->sales->account_id != null) {
    ...
    function pendignOrder()  {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Pendign Order Record";  
        $data['content'] = $this->load->view('Administrator/order/pending_order', $data, TRUE);
        $this->load->view('Administrator/index', $data); 
    }
    ...
    public function cancelOrder(){
        ...
        try{
            ...
            $this->db->set('Status', 'c')->where('SaleMaster_IDNo', $saleId)->update('tbl_saledetails');

            /*deliver Sale Master Data*/
            $this->db->set('Status', 'c')->where('SaleMaster_SlNo', $saleId)->update('tbl_salesmaster');

            // Bank transection update
            if($sale->payment_type == 'Bank') {
                $this->db->set('status', 0)->where('sales_master_id', $saleId)->update('tbl_bank_transactions');
            }

            $res = ['success'=>true, 'message'=>'Order Cancel Successful!'];
    ...
    public function  deleveredOrder(){
        ...
        try{
            ...
            // Bank transection update
            if($sale->payment_type == 'Bank') {
                $this->db->set('status', 1)->where('sales_master_id', $saleId)->update('tbl_bank_transactions');
            }
