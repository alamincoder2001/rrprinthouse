					...
					<div style="width:50%;border:1px solid black;border-left:none;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="advanceSalaries.length > 0">
									<tr>
										<td class="sub-heading">Salary Advance</td>
										<td class="sub-value">{{ totalAdvanceSalary }}</td>
									</tr>
									<tr v-for="payment in advanceSalaries">
										<td>{{ payment.Employee_Name }}</td>
										<td>{{ payment.totalAmount | decimal }}</td>
									</tr>
								</template>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalAdvanceSalary(){
				return this.advanceSalaries.reduce((prev, curr) => {
					return prev + parseFloat(curr.totalAmount)
				}, 0).toFixed(2);
			},
			totalInitialLoan(){
			...
			totalOut(){
				return parseFloat(this.totalPurchase) +
					...
					parseFloat(this.totalAdvanceSalary) +
		...
		methods: {
			...
			getAdvanceSalary(){
				axios.post('/get_salary_advance', this.filter)
				.then(res => { 
					let payments = res.data;
					payments = _.groupBy(payments, 'employee_id');
					payments = _.toArray(payments);
					payments = payments.map(payment => {
						payment[0].totalAmount = payment.reduce((p, c) => { return p + parseFloat(c.amount) }, 0);
						return payment[0];
					})
					this.advanceSalaries = payments;
