				...
				<table 
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:center;">
								...
								<a href="" title="Cancel Order" @click.prevent="cancelOrder(sale.SaleMaster_SlNo)"><i class="fa fa-ban"></i></a>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ parseFloat(sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_SubTotalAmount)}, 0)).toFixed(2) }}</td>
							<td style="text-align:right;">{{ parseFloat(sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_TaxAmount)}, 0)).toFixed(2) }}</td>
							<td style="text-align:right;">{{ parseFloat(sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_TotalDiscountAmount)}, 0)).toFixed(2) }}</td>
							<td style="text-align:right;">{{ parseFloat(sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_Freight)}, 0)).toFixed(2) }}</td>
							<td style="text-align:right;">{{ parseFloat(sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_TotalSaleAmount)}, 0)).toFixed(2) }}</td>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			cancelOrder(saleId){
				let deleteConf = confirm('Are you sure? you want to Cancel this order?');
				if(deleteConf == false){
					return;
				}
				axios.post('/cancel_order', {saleId: saleId})
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success){
