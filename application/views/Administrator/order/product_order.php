...
<div id="sales" class="row">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			...
			<div class="form-group">
				<label class="col-xs-4 col-md-1 control-label no-padding-right"> Order By </label>
				<div class="col-xs-8 col-md-2">
			...
			<div class="form-group">
				<label class="col-xs-4 col-md-1 control-label no-padding-right"> Order From </label>
				<div class="col-xs-8 col-md-2">
					...
					<div class="row">
						<div class="col-xs-12 col-md-6">
							<div class="form-group clearfix" style="margin-bottom: 8px;">
								...
								<div class="col-xs-8">
									<label for="Retail">
										<input type="radio" id="Retail" name="salesType" value="retail" v-model="sales.salesType" v-on:change="onSalesTypeChange"> Retail &nbsp;
									</label>
									<label for="Wholesale">
										<input type="radio" id="Wholesale" name="salesType" value="wholesale" v-model="sales.salesType" v-on:change="onSalesTypeChange"> Wholesale
									</label>
								</div>
							</div>
							<div class="form-group">
						...
						<div class="col-xs-12 col-md-6">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group clearfix" style="margin-bottom: 8px;">
									...
									<div class="col-xs-9">
										<label for="pro">
											<input type="radio" id="pro" name="service" value="false" v-model="sales.isService" v-on:change="onServiceChange"> Product
										</label>
										<label for="service">
											<input type="radio" id="service" name="service" value="true" v-model="sales.isService" v-on:change="onServiceChange"> Service
										</label>
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:10%;color:#000;">Pro. Code</th>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label for="paymentType" class="col-xs-12 control-label no-padding-right">Payment Type</label>
												<div class="col-xs-12">
													<select style="padding: 0px 5px;" class="form-control" id="paymentType" v-model="sales.payment_type">
									...
									<tr style="display: none;" :style="{display: sales.payment_type == 'Bank' ? '' : 'none'}">
										<td>
											<div class="form-group">
												<label for="payment_type" class="col-xs-12 control-label no-padding-right">Select Account</label>
												<div class="col-xs-12">
													<v-select v-bind:options="accounts" label="dispaly_account" v-model="selectedAccount" placeholder="Bank Account"></v-select>
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-xs-6">
													<a class="btn btn-info btn-sm" href="/order_entry" style="color: black!important;margin-top: 0px;width:100%;padding:5px;font-weight:bold;">New Order</a>
												</div>
											</div>
										</td>
									</tr>
	...
	new Vue({
		...
		data() {
			return {
				...
				selectedColor: null,
				othercolors: [],
				selectedOtherColor: null,
				bothcolors: [],
				selectedBothColor: null,
				productPurchaseRate: '',
				productStockText: '',
				productStock: '',
		...
		methods: {
			...
			colorOnChange(val) {
				if (this.selectedColor == null && this.selectedOtherColor == null && this.selectedBothColor == null) {
					document.querySelector('#bothcolor [type="search"]').removeAttribute('disabled', true);
					document.querySelector('#othercolor [type="search"]').removeAttribute('disabled', true);
					document.querySelector('#color [type="search"]').removeAttribute('disabled', true);
					this.selectedProduct.Product_SellingPrice = 0;
				} else {
					if (val == 'color') {
						document.querySelector('#othercolor [type="search"]').setAttribute('disabled', true);
						document.querySelector('#bothcolor [type="search"]').setAttribute('disabled', true);
						this.selectedProduct.Product_SellingPrice = this.selectedColor.amount;
					}
					if (val == 'othercolor') {
						document.querySelector('#color [type="search"]').setAttribute('disabled', true);
						document.querySelector('#bothcolor [type="search"]').setAttribute('disabled', true);
						this.selectedProduct.Product_SellingPrice = this.selectedOtherColor.amount;
					}
					if (val == 'bothcolor') {
						document.querySelector('#othercolor [type="search"]').setAttribute('disabled', true);
						document.querySelector('#color [type="search"]').setAttribute('disabled', true);
						this.selectedProduct.Product_SellingPrice = this.selectedBothColor.amount;
					}
			...
			getAccounts() {
				axios.get('/get_bank_accounts').then(res => {
					this.accounts = res.data.map(item => {
						item.dispaly_account = item.account_name + '-' + item.account_number + '-' + item.bank_name;
						return item;
					});
				})
			},
			...
			addToCart() {
				let product = {
					...
					color: this.selectedColor !=null ? this.selectedColor.color_SiNo:'',
					othercolor: this.selectedOtherColor !=null ? this.selectedOtherColor.color_SiNo:'',
					bothcolor: this.selectedBothColor !=null ? this.selectedBothColor.color_SiNo:'',
				...
				let cartInd = this.cart.findIndex(p => p.productId == product.productId && p.color == product.color && p.othercolor == product.othercolor && p.bothcolor == product.bothcolor && p.note == product.note);
				if (cartInd > -1) {
			...
			clearProduct() {
				...
				this.selectedOtherColor = null
				this.selectedBothColor = null
				this.productStock = '';
				...
				await axios.post('/get_orders', {
					...
					if (sales.payment_type == 'Bank') {
						...
						this.selectedAccount = {
							...
							dispaly_account: sales.account_name + '-' + sales.account_number + '-' + sales.bank_name
						}
					}
					...
					r.saleDetails.forEach(product => {
						let cartProduct = {
							...
							note: product.note,
							color:product.color,
							othercolor:product.othercolor,
							bothcolor:product.bothcolor,
