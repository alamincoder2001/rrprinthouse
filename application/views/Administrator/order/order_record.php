	...
	<div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group">
					<label>Status</label>
					<select class="form-control" v-model="Status" @change="sales = []">
						<option value="">All</option>
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'area' && districts.length > 0 ? '' : 'none'}">
					<label>Area</label>
					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:center;">{{ sale.saleDetails[0].note ?? '--' }}</td>
								<td>
									<span v-if="sale.Status == 'p'" class="badge badge-danger">Pending</span>
									<span v-else-if="sale.Status == 'c'" class="badge badge-warning">Cancel</span>
									<span v-else class="badge badge-success">Confirm</span>
								</td>
								<td style="text-align:center;">
									...
									<a v-if="sale.Status == 'p'" href="javascript:" title="Edit Order" @click="checkReturnAndEdit(sale)"><i class="fa fa-edit"></i></a>

									<a href="" v-if="sale.Status == 'p'" title="Order Delivered" @click.prevent="deliveryOrder(sale.SaleMaster_SlNo)"><i class="fa fa-truck"></i></a>
							...
							<tr v-for="(product, sl) in sale.saleDetails.slice(1)">
								...
								<td style="text-align:right;">{{ product.note ?? '--' }}</td>
								<td></td>
							</tr>
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:center;">
								...
								<a v-if="sale.Status == 'p'" href="" title="Order Delivered" @click.prevent="deliveryOrder(sale.SaleMaster_SlNo)"><i class="fa fa-truck"></i></a>
		...
		methods: {
			...
			getSalesRecord(){
				let filter = {
					...
					districtId: this.selectedDistrict == null || this.selectedDistrict.District_SlNo == '' ? '' : this.selectedDistrict.District_SlNo,
					Status: this.Status,
			...
			async print(){
				...
				let areaText = '';
				if(this.selectedDistrict != null && this.selectedDistrict.District_SlNo != '' && this.searchType == 'category'){
					areaText = `<strong>Area: </strong> ${this.selectedDistrict.District_Name}`;
				}
				...
				let reportContent = `
					<div class="container">
						...
						<div class="row">
							<div class="col-xs-6">
								${userText} ${customerText} ${employeeText} ${productText} ${categoryText} ${$areaText}
							</div>
