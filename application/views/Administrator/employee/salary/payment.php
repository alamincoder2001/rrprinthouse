				...
				<table class="table table-bordered">
					...
					<tbody>
						<tr v-for="(employee, i) in employees" v-bind:style="{background: employee.net_payable != employee.payment ? 'orange' : ''}">
							...
							<td style="text-align: center;">{{ employee.advance }}</td>
	...
	new Vue({
		...
		methods: {
			...
			calculateNetPayable(employee){
				let payable = ((parseFloat(employee.salary) + parseFloat(employee.benefit)) - (parseFloat(employee.advance) + parseFloat(employee.deduction))).toFixed(2);
			...
			async getEmployees() {
				...
				if(this.payment){
					...
					await axios.post('/get_employees_for_salary', {monthId: month_id}).then(res => {
						let employees = res.data;

						employees.map(employee=>{
							employee.salary = employee.salary_range;
							employee.advance = employee.advance_salary;
							employee.benefit = 0;
							employee.deduction = 0;
							employee.net_payable = employee.net_payable;
							employee.payment = employee.net_payable;
							employee.comment = '';
							return employee;
						});
