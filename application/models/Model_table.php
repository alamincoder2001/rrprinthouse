...
class Model_Table extends CI_Model{
    ...
    // salary advance code
    public function generateSalaryAdvanceCode(){
        $paymentCode = "SA00001";
        
        $lastPayment = $this->db->query("select * from tbl_salary_advance order by id desc limit 1");
        if($lastPayment->num_rows() != 0){
            $newPaymentId = $lastPayment->row()->id + 1;
            $zeros = array('0', '00', '000', '0000');
            $paymentCode = 'SA' . (strlen($newPaymentId) > count($zeros) ? $newPaymentId : $zeros[count($zeros) - strlen($newPaymentId)] . $newPaymentId);
        }
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                ...
                where sa.branch_id = " . $this->session->userdata('BRANCHid') . "
                and sa.status = 'a'
                and sa.type = 'cash'
                " . ($date == null ? "" : " and sa.date < '$date'") . "
            ...
            (
                select paid_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + employee_advance_salary + loan_payment + invest_payment + buy_asset
            ) as total_out,
            (
    ...
    public function getBankTransactionSummary($accountId = null, $date = null){
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select (ba.initial_balance + total_deposit + total_received_from_customer + total_received_from_supplier) - (total_withdraw + total_paid_to_customer + total_paid_to_supplier + employee_advance_salary)
                ) as balance
